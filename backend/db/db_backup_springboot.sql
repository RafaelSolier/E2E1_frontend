...
INSERT INTO public.users (avg_rating, created_at, email, first_name, last_name, password, phone_number, role, trips, updated_at) VALUES (0, '2024-05-21 00:17:56.415755-05', 'john.doe@utec.edu.pe', 'John', 'Doe', '$2a$10$MpZ1CKmZyMHTUnMKhzz84e5OTMpxvTVOSPx.c3uHiVFkOcd1tnqbm', '999999999', 2, 0, NULL),(0, '2024-05-20 23:32:22.874029-05', 'jmonja@utec.edu.pe', 'Jeffrey', 'Monja', '$2a$10$WloHutd2jTVy6AKqKwq0se3DuOkCh7b.eCGiZlNZIlQFu/JwxEDPi', '987654321', 1, 0, NULL);

INSERT INTO public.vehicle (brand, capacity, fabrication_year, license_plate, model) VALUES ('Toyota', 5, 2020, 'ABC-123', 'Yaris');
...
-- Sincronizar todas las secuencias al final del backup
SELECT setval('vehicle_id_seq', COALESCE((SELECT MAX(id) FROM vehicle), 1), true);
SELECT setval('users_seq', COALESCE((SELECT MAX(id) FROM users), 1), true);
SELECT setval('coordinate_id_seq', COALESCE((SELECT MAX(id) FROM coordinate), 1), true);
SELECT setval('ride_id_seq', COALESCE((SELECT MAX(id) FROM ride), 1), true);
SELECT setval('review_id_seq', COALESCE((SELECT MAX(id) FROM review), 1), true);
